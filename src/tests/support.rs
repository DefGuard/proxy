...
use crate::{
    ...
    proto::{CoreError, PublicSettings, core_request, core_response},
};

pub(crate) fn cookie_key() -> Arc<RwLock<Option<Key>>> {
...
/// An API router backed by a fake Core that answers every request through `core`.
pub(crate) fn app_with_fake_core<F>(core: F) -> Router
where
    F: Fn(core_request::Payload) -> core_response::Payload + Send + 'static,
{
    ...
    tokio::spawn(async move {
        while let Some(Ok(request)) = requests.recv().await {
            ...
            responder.resolve_test_response(request.id, response);
        }
    });
